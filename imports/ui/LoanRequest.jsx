...
function LoanRequest({ role, email }) {
  ...
  return (
    <div className='flex flex-col w-full'>
      <h1 className='text-2xl font-light text-green-700'>Loan Request</h1>
      <div className='flex mt-12 md:flex-row flex-col'>
        ...
        >
          <div className='flex flex-col'>
            <label className='mb-2 text-green-700'>Amount</label>
            <input
              type='number'
          ...
          <button
            type='submit'
            className='bg-green-700 p-2 mt-6 text-white rounded-lg'
          >
            Request
          </button>
        </form>
        <div className='bg-gray-300 border-l p-4 ml-2 w-full'>
          <h1 className='text-md text-green-700'>Past Loans</h1>
          <div className='w-full'>
            {loans.map((loan) => {
              return (
                <div
                  className='flex w-full justify-between mt-4 items-center'
                  key={loan._id}
                >
                  ...
                  <p className='flex items-center justify-center sm:text-sm md:text-md'>
                    {' '}
                    <p
                      className={`w-3 h-3 ${
                        loan.status === 'Pending' ? 'bg-red-500' : 'bg-green-400'
                      } rounded-xl  mr-2 flex`}
                    ></p>{' '}
                    {loan.status}
                  </p>
                </div>
